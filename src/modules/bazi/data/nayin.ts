 ...
 *
 *   ⚠ 4 条名称与古籍原文有别（tyme4ts 现代用字 vs 三命通会原名）：
 *   - 金箔金（code）≈ 金泊金（raw）
 *   - 沙中金（code）≈ 砂中金（raw）
 *   - 泉中水（code）≈ 井泉水（raw）
 *   - 沙中土（code）≈ 砂中土（raw）
 *   策略：`classical` 字段严格忠于 raw 原字，`source` 保持干净仅含书名·卷·行号；
 *   code/raw 差异仅在对应条目上方用代码注释说明，不污染用户可见文案。
 *
 * 🔒 全项目规则（classical / source 书写约束）：
 *   1. `classical` 字迹简繁 = raw 原文字迹，**不做简繁转换**。
 *      - 本表 raw（《三命通会 · 卷一》维基文库版）为**简体**，故 classical 用简体。
 *      - 反例：`patternLongInterpret.ts` 的 raw（《子平真诠评注》ctext.org 版）为**繁体**，
 *        故其 classical 用繁体。按 raw 字迹定，不按团队偏好定。
 *   2. `source` 只写"《书名》· 卷/篇 · L起行-止行"，不混入任何代码侧备注/差异注记。
 *      差异说明全部走**代码注释**（条目上方 `//`），不向用户展示。
 *
 * short:     ≤20 字一句话概括，用于四柱表纳音单元直接提示
 * long:      80-120 字详细释义，用于 tooltip / 详情页
 * classical: 古籍原文主旨句（字迹忠于 raw，≤ 40 字），UI 在 long 下方用小字引用
 ...
 *
 * 2026-04-22 字迹修正：
 *   - 初版 30 条 classical 误用繁体（因 Nayin 与 Pattern 共用 `classical` 字段而沿袭了 patternLongInterpret 的繁体策略）
 *   - 本表 raw（三命通会 · 卷一 · 维基文库版）实为**简体**，故 classical 逐条改回简体，严格对齐 raw 字迹
 *   - 新规则见本注释头部 🔒 「全项目规则」，同步落在 2026-04-22-03 TODO § 0 「P-5 派生规则」
 */
...
export interface NayinMeaning {
  ...
  long: string
  /** 古籍原文主旨句（字迹忠于 raw 原文，≤ 40 字），UI 在 long 下方用小字引用 */
  classical: string
  /** 原文锚点（《三命通会》· 卷一 · L###） */
  source: string
  /** 是否古籍可溯源（本表统一 true） */
 ...
 * - short 不超过 20 字
 * - long 控制在 80-120 字
 * - classical 不超过 40 字（字迹忠于 raw 原文，句读保留原文，末尾用"。"）
 *
 * 文风约束：
...
export const NAYIN_MEANING: Record<string, NayinMeaning> = {
  ...
  海中金: {
    ...
    classical: '海中金者，宝藏龙宫，珠孕蛟宝，出现虽假于空冲，成器无借乎火力。',
  ...
  剑锋金: {
    ...
    classical: '剑锋金者，白帝司权，刚由百炼，红光射于斗牛，白刃凝于霜雪。',
  ...
  白蜡金: {
    ...
    classical: '白蜡金者，昆山片玉，洛浦遗金，交栖日月之光，凝聚阴阳之气。',
    source: '《三命通会》· 卷一 · 论纳音取象 · L289-291',
    isClassical: true,
    auditStatus: 'verified',
    element: '金',
  },
  // ⚠ code key「沙中金」对应 raw 原名「砂中金」（现代字 vs 古籍原字），
  //    classical 严格忠于 raw 原文，source 保持干净不展示这类差异注记
  沙中金: {
    short: '淘沙见金，因人始贵',
    long: '刚形布地、宝质藏沙，直待陶洗、淘汰方为珍宝。主早年辛劳积累，后凭贵人提携或机缘磨砺而显，属于"因人成事"之格局。宜配清贵禄马、火土得中以助其成器，忌水泛则沙散、刑冲破则财折。',
    classical: '砂中金者，刚形布地，宝质藏砂，直教陶洗为珍，必须因人始贵。',
    source: '《三命通会》· 卷一 · 论纳音取象 · L293-295',
    isClassical: true,
    auditStatus: 'verified',
    element: '金',
  },
  // ⚠ code key「金箔金」对应 raw 原名「金泊金」（现代字 vs 古籍原字），
  //    classical 严格忠于 raw 原文，source 保持干净不展示这类差异注记
  金箔金: {
    short: '金薄有光，依人而贵',
    long: '润色杯盘、增光宫室之薄金，贴附于器物之上方显其彩。主性情精致外向、善于借势借力，贵气多来自合作与依附而非独立锻造。宜与别金、厚土相佐以成其薄而不弱之质，忌遇炉火粗煅、猛器破损。',
    classical: '金泊金者，润色杯盘，增光宫室，打薄须借乎别金，描彩必假乎水力。',
    source: '《三命通会》· 卷一 · 论纳音取象 · L285-287',
    isClassical: true,
    auditStatus: 'verified',
    element: '金',
  },
  钗钏金: {
    ...
    classical: '钗钏金者，美容首饰，增光腻肌，偎红倚翠之珍，枕玉眠香之宝。',
  ...
  炉中火: {
    ...
    classical: '炉中火者，天地为炉，阴阳为炭，腾光辉于宇宙，成陶冶于乾坤。',
  ...
  山头火: {
    ...
    classical: '山头火者，野焚燎原，延烧极目，依稀天际斜晖，仿佛山头落日。',
  ...
  霹雳火: {
    ...
    classical: '霹雳火者，一缕毫光，九天号令，电掣金蛇之势，云驱铁马之奔。',
  ...
  山下火: {
    ...
    classical: '山下火者，草间熠耀，花理荧煌，寒林缀叶之光，隔幔点衣之彩。',
  ...
  覆灯火: {
    ...
    classical: '覆灯火者，金盏光辉，玉台吐艳，照日月不照之处，明天地未明之时。',
  ...
  天上火: {
    ...
    classical: '天上火者，温暖山海，辉光宇宙，阳德丽天之照，阴精离海之明。',
  ...
  大林木: {
    ...
    classical: '大林木者，枝干撼风，柯条撑月，耸壑昂霄之德，凌云蔽日之功。',
  ...
  杨柳木: {
    ...
    classical: '杨柳木者，隋堤袅娜，汉苑轻盈，万缕不蚕之丝，千条不针之带。',
  ...
  松柏木: {
    ...
    classical: '松柏木者，泼雪凑霜，参天覆地，风撼笙篁之奏，雨余旌旆之张。',
  ...
  平地木: {
    ...
    classical: '平地木者，初生萌蘖，始发枝条，唯资雨露之功，不喜雪霜之积。',
  ...
  桑柘木: {
    ...
    classical: '桑柘木者，缯彩之基，绮罗根本，士民飘飘之袂，圣贤楚楚之衣。',
  ...
  石榴木: {
    ...
    classical: '石榴木者，性辛如姜，花红似火，数颗枝头，累累多子，房内莹莹。',
  ...
  涧下水: {
    ...
    classical: '涧下水者，山环细浪，雪涌飞端，相连南北之流，对峙坎离之脉。',
  ...
  大溪水: {
    ...
    classical: '大溪水者，惊涛薄岸，骇浪浮天，光涵万里之宽，碧倒千山之影。',
  ...
  长流水: {
    ...
    classical: '长流水者，混混无穷，滔滔不竭，就下必纳于东南，顺流自归于辰巳。',
  ...
  天河水: {
    ...
    classical: '天河水者，乱洒六野，密沛千郊，淋淋泻下银河，细细飞来碧落。',
    source: '《三命通会》· 卷一 · 论纳音取象 · L389-391',
    isClassical: true,
    auditStatus: 'verified',
    element: '水',
  },
  // ⚠ code key「泉中水」对应 raw 原名「井泉水」（现代字 vs 古籍原字），
  //    classical 严格忠于 raw 原文，source 保持干净不展示这类差异注记
  泉中水: {
    short: '井泉清冽，惠及众人',
    long: '寒泉清冽、取养不穷，八家凿之以同饮、万民资之以生活，属井泉之水。主性情公道清正，利众而少私心，适合公共事务与服务性工作。喜金助生源、喜阴阳互见之火，忌源枯或污浊乱流。',
    classical: '井泉水者，寒泉清冽，取养不穷，八家凿之以同饮，万民资之以生活。',
    source: '《三命通会》· 卷一 · 论纳音取象 · L393-395',
    isClassical: true,
    auditStatus: 'verified',
    element: '水',
  },
  大海水: {
    ...
    classical: '大海水者，总纳百川，汪洋无际，包括乾坤之大，升沉日月之光。',
  ...
  路旁土: {
    ...
    classical: '路旁土者，大地连途，平田万顷，禾稼赖以资生，草木由之畅茂。',
  ...
  城头土: {
    ...
    classical: '城头土者，天京玉垒，帝里金城，龙盘千里之形，虎踞四维之势。',
  ...
  屋上土: {
    ...
    classical: '屋上土者，埏埴为林，水火既济，盖蔽雪霜之积，震凌风雨之功。',
  ...
  壁上土: {
    ...
    classical: '壁上土者，恃栋依梁，兴门立户，却暑御寒之德，遮霜护雪之功。',
  ...
  大驿土: {
    ...
    classical: '大驿土者，堂堂大道，坦坦平途，九州无所不通，万国无所不至。',
    source: '《三命通会》· 卷一 · 论纳音取象 · L369-371',
    isClassical: true,
    auditStatus: 'verified',
    element: '土',
  },
  // ⚠ code key「沙中土」对应 raw 原名「砂中土」（现代字 vs 古籍原字），
  //    classical 严格忠于 raw 原文，source 保持干净不展示这类差异注记
  沙中土: {
    short: '潮汐所聚，变迁有时',
    long: '浪回所积、波渚而成，龙蛇盘隐之宫、陵谷变迁之地，属河沙淤积之土。主命局多变迁与流动，适合顺势调整、借势而行。喜清金（钗砂剑箔）涵养其质；忌路旁、大驿之土相扰，使其反成泥泞。',
    classical: '砂中土者，浪回所积，波渚而成，龙蛇盘隐之宫，陵谷变迁之地。',
    source: '《三命通会》· 卷一 · 论纳音取象 · L361-363',
    isClassical: true,
    auditStatus: 'verified',
    element: '土',
  },
