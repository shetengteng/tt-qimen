# tt-qimen 宣传片 · 开发者版 · 总分镜脚本(v1.5.4)

**版本** v1.5.4 · 在 v1.5.3 基础上,**强化 S0 GitHub 截图展示**:S0 时长从 6s → 9s(+3s),Ken Burns 滚动幅度从 -18% → -40%(可滚到 README "技术栈"表 + 桌面端 Tauri 2 介绍),静态停留 1.2s 让观众先看清仓库名再开始滚。后续 S1~S11 全部 +3s 顺延,**总时长 120s → 123s**
**总时长** 123 秒(目标 90-120 秒区间,**为了让观众看清 GitHub README 而拉长 3s**)
...
| # | 起点 | 时长 | 镜头标题 | 信息焦点 |
|---|---|---|---|---|
| **S0** | **00:00** | **9s** | **GitHub 真截图 · 开场(v1.5.4 强化)** | 浏览器壳 + 真实仓库截图 + Ken Burns 慢推到 README 技术栈表 + 桌面端介绍 |
| S1 | 00:09 | 6s | **启门问卜** | 与 v1 风格完全一致:"古人问天 · 今人问己" |
| S2 | 00:15 | 9s | **八字排盘 demo(mock)** | 仿古 4 柱呈现 + `tyme4ts` 角标 |
| S3 | 00:24 | 8s | **古籍考据** | 4 本古籍仿古页面飞过 + "一切有迹可循" |
| S4 | 00:32 | 11s | **8 大模块矩阵** | 模块卡 + 每张底部核心库角标 |
| S4b | 00:43 | 10s | **真实产品页轮播** | 5 张真实站点截图轮播:首页 → 八字 → 紫微 → 黄历 → 设置 |
| **S5** | **00:53** | **14s** | **AI 解读 · v1.4 改:左半真截图 + 右半真 markdown** | **左半 `app-bazi-result.png`**(带 query 自动排好的真实页面)+ 浏览器壳 + `LIVE` 角标;**右半 4 块逐段揭示的真 markdown 输出**(命盘总览/格局/喜忌/神煞/免责声明),8 家 LLM chip 切换 |
| S6 | 01:07 | 10s | **双主题(真截图)** | 左右各嵌入一张真实截图 `theme-guofeng.png` / `theme-minimal.png` |
| **S7** | **01:17** | **10s** | **三种形态 · v1.4 改:真截图 × 3** | Web / macOS / Windows 三个窗口外壳各自嵌入 `app-home.png` 真截图 + 平台标签(GitHub Pages / .dmg / .msi),顶部主标"一份代码 · 三种形态" |
| S8 | 01:27 | 10s | **隐私本地** | 零后端 / BYOK / localStorage |
| S9 | 01:37 | 12s | **核心技术栈墙** | 13 个技术 logo + 标签平铺呈现 |
| **S10** | **01:49** | **10s** | **项目设计思路(v1.4 新)** | 4 层架构图(UI → Application → Domain Core → AI Layer · BYOK)逐层入场 + 层间虚线箭头;右侧 aside 卡列 4 条设计原则 |
| S11 | 01:59 | 4s | **CTA · Star 与贡献** | MIT License + Made with care |

---

## 详细分镜

### S0 · 00:00–00:09 · GitHub 真截图 · 开场(v1.5.4 强化)
...
| **动效(v1.5.4)** | 截图 `opacity 0→1`(1.2s 入场);**0–1.2s 静态**,让观众看清顶部 `shetengteng / tt-qimen Public` + About 简介;**1.2s 后开始** 7.5s Ken Burns 缓动:`translateY(0) → -4%(过渡)→ -40%` + `scale(1.00 → 1.05)`,**自然滚到 README 的"技术栈"表(Vue 3.4 / Vite 5 / TypeScript / Pinia / Tailwind / iztro / tyme4ts...)和"桌面端(Tauri 2)"章节**,让开发者直接看清项目骨架 |
| **屏上文字** | (仅浏览器壳与右上徽,无锚点说明) |
| **旁白(zh)** | (静默 0.8s)"一份开源的中式命理 App,代码托管在 GitHub,MIT License。" |
| **字幕(2 行)** | `github.com/shetengteng/tt-qimen`(0.8–4.5s) / 一份开源的中式命理 App · MIT License(4.7–9.0s) |
| **配音语气** | 男低音,平直陈述,像电影片头报字幕 |
| **依赖资产** | `promo/v2/assets/github-repo.png`(Playwright 截自真实 GitHub 仓库,1440×8668) |
| **v1.5.4 改动** | S0 时长 6s → 9s(+3s);Ken Burns 滚动幅度 -18% → -40%(让 README 技术栈表 + Tauri 2 桌面端介绍完整露出);加入 1.2s 静态停留;后续 S1~S11 全部 +3s 顺延,总时长 120s → 123s |

### S1 · 00:09–00:15 · 启门问卜 (6s)
...
### S5 · 00:53–01:07 · AI 解读演示(v1.4 改:左半真截图 + 右半真 markdown) (14s)
...
### S6 · 01:07–01:17 · 双主题(v1.3 改:真截图) (10s)
...
### S7 · 01:17–01:27 · 三种形态(v1.4 改:真截图 × 3) (10s)
...
### S10 · 01:49–01:59 · 项目设计思路(v1.4 新增) (10s)
