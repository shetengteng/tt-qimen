#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""姓名学模块基线截图：覆盖 2 主题 × 3 语言 × (输入态 / 结果态) × 桌面/移动。

只抓 12-16 张关键截图用于人工/AI 视觉巡检：
  desktop-{theme}-{lang}-input.png
  desktop-{theme}-{lang}-result.png
  mobile-{theme}-{lang}-result.png
"""
...
OUT_DIR = Path("design/screenshots/2026-04-25-xingming-baseline").resolve()
...
def shoot_desktop(page: Page, theme: str, lang: str) -> None:
    ...
    goto_xingming(page)
    shot(page, f"desktop-{short_t}-{lang}-input")
